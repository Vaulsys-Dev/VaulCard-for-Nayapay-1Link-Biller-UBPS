...
public class FinanceManager {
    ...
    public static boolean WalletTransaction(WalletCMSWsEntity wsmodel, CMSAccount srcaccount, CMSAccount destaccount)
    {
        try {
            ...
            // Asim Shahzad, Date : 22nd Feb 2022, Tracking ID : VC-NAP-202202104
            srcbalanceLog.setTransDT(Util.hasText(wsmodel.getTransdatetime()) ? wsmodel.getTransdatetime() : (new SimpleDateFormat(WebServiceUtil.transdatetimeFormat)).format(new Date()));
            // ==================================================================
            ...
            // Asim Shahzad, Date : 22nd Feb 2022, Tracking ID : VC-NAP-202202104
            destbalanceLog.setTransDT(Util.hasText(wsmodel.getTransdatetime()) ? wsmodel.getTransdatetime() : (new SimpleDateFormat(WebServiceUtil.transdatetimeFormat)).format(new Date()));
            // ==================================================================
...
//            account.setAvailableBalance((AcctAvailBalance + (DepositAmount - (NayaPayChargeAmount + BankCharges + BankTaxAmount + SrcChargeAmount))) + "");
    ...
    public static boolean MerchantRefund(WalletCMSWsEntity wsmodel, WalletCMSWsEntity origTxn, CMSAccount merchantWallet, CMSAccount userWallet)
    {
        try {
            ...
            balanceLog2.setWallet(userWallet);
            balanceLog2.setChannelid(wsmodel.getChannelid());
            balanceLog2.setAmount(TxnAmount + "");
            balanceLog2.setOriginalbalance(userWalletAvailBalance + "");
            balanceLog2.setUpdatedbalance(userWallet.getAvailableBalance());
            balanceLog2.setTxnname(wsmodel.getServicename());
            balanceLog2.setTransaction(wsmodel);
            balanceLog2.setTxnnature(TxnFlag.CREDIT);
            balanceLog2.setCreatedate(new Date());
            balanceLog2.setTransDT(Util.hasText(wsmodel.getTransdatetime()) ? wsmodel.getTransdatetime() : (new SimpleDateFormat(WebServiceUtil.transdatetimeFormat)).format(new Date()));
            // ==================================================================
    ...
    public static boolean MerchantDebitCard(WalletCMSWsEntity wsmodel, CMSAccount sourceWallet, CMSAccount destWallet)
    {
        try {
            ...
            logger.info("Updating Destination Wallet Balance Log ...");
            WalletBalanceLog balanceLog2 = new WalletBalanceLog();
            balanceLog2.setWallet(destWallet);
            balanceLog2.setChannelid(wsmodel.getChannelid());
            balanceLog2.setAmount(TxnAmount + "");
            balanceLog2.setOriginalbalance(userWalletAvailBalance + "");
            balanceLog2.setUpdatedbalance(destWallet.getAvailableBalance());
            balanceLog2.setTxnname(wsmodel.getServicename());
            balanceLog2.setTransaction(wsmodel);
            balanceLog2.setTxnnature(TxnFlag.CREDIT);
            balanceLog2.setCreatedate(new Date());
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    public static CMSEMIWallet GetRevenueWallet(String currency)
    {
        ...
        dbQuery = "from " + CMSEMIWallet.class.getName() + " c where c.Currency= :CURR" + " and c.category = :CAT ";
        params = new HashMap<String, Object>();
    ...
    public static CMSEMIAccountCollection GetEMICollectionAccount(String currency, AccType category)
    {
        ...
        //Raza Wallet Must be verified before every transaction, as if it is blocked, every thing should be rejected
        dbQuery = "from " + CMSEMIAccountCollection.class.getName() + " c where c.Currency= :CURR" + " and c.category = :CAT ";
        params = new HashMap<String, Object>();
        params.put("CURR", currency);
    ...
    public static CMSBusinessWallet GetBusinessWallet(String currency, String category)
    {
        ...
        //Raza Wallet Must be verified before every transaction, as if it is blocked, every thing should be rejected
        //m.rehman: 10-1-2022, Nayapay Optimization
        //dbQuery = "from " + CMSBusinessWallet.class.getName() + " c where c.Currency= :CURR" + " and c.category = :CAT ";
        dbQuery = "from " + CMSBusinessWallet.class.getName() + " c where c.id = 1 ";
        params = new HashMap<String, Object>();
        //m.rehman: 10-1-2022, Nayapay Optimization
        //params.put("CURR", currency);
    ...
    public static CMSEMIAccountCollection GetBillerAccount(String companyid, String currency)
    {
        ...
        //Raza Wallet Must be verified before every transaction, as if it is blocked, every thing should be rejected
        dbQuery = "from " + CMSEMIAccountCollection.class.getName() + " c where c.Currency= :CURR" + " and c.category = :CAT and c.userid = :USERID ";
        params = new HashMap<String, Object>();
        params.put("CURR", currency);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
                                                  ...
                                                  WalletCMSWsEntity wsEntity)
    {
        try {
            ...
            // Asim Shahzad, Date : 23rd Feb 2022, Tracking ID : VC-NAP-202202104
            srcbalanceLog.setTransDT(Util.hasText(wsmodel.getTransdatetime()) ? wsmodel.getTransdatetime() : (new SimpleDateFormat(WebServiceUtil.transdatetimeFormat)).format(new Date()));
            // ==================================================================
    ...
    public static boolean RefundMerchantTransaction(WalletCMSWsEntity wsmodel, CMSAccount account, WalletCMSWsEntity wsEntity)
    {
        try {
            ...
            balanceLog.setUpdatedbalance(account.getAvailableBalance());
            balanceLog.setTxnname(wsmodel.getServicename());
            // Asim Shahzad, Date : 24th Feb 2022, Tracking ID : VC-NAP-202202104
            //balanceLog.setTransaction(wsmodel);
            balanceLog.setTransaction(wsEntity);
            // ==================================================================
            balanceLog.setTxnnature(TxnFlag.CREDIT);
            balanceLog.setCreatedate(new Date());
...
//            }
                                                          ...
                                                          WalletCMSWsEntity wsEntity)
    {
        try {
            ...
            // Asim Shahzad, Date : 23rd Feb 2022, Tracking ID : VC-NAP-202202104
            //destbalanceLog.setTransaction(wsmodel);
            destbalanceLog.setTransaction(wsEntity);
            // ==================================================================
            destbalanceLog.setTxnnature(TxnFlag.CREDIT);
            destbalanceLog.setCreatedate(new Date());
            ...
            // Asim Shahzad, Date : 24th Feb 2022, Tracking ID : VC-NAP-202202104
            //srcbalanceLog.setTransaction(wsmodel);
            srcbalanceLog.setTransaction(wsEntity);
            // ==================================================================
            srcbalanceLog.setTxnnature(TxnFlag.DEBIT);
            srcbalanceLog.setCreatedate(new Date());
...
//            balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //m.rehman: 20-04-2022, VC-NAP-202204151 - OnelinkTopupBillPayment Transaction not handed in FMS - Production
    public static boolean TopupBillPaymentFromSP(WalletCMSWsEntity wsmodel)
    {
        ...
        try {
            ...
            String queryStr = "{ CALL PACKAGE_TRANSACTION.spOnelinkTopupBillPayment(?,?,?,?,?) }";
            callableStatement = GeneralDao.Instance.getCurrentSession().connection().prepareCall(queryStr);
            callableStatement.setLong(1, wsmodel.getId());
            callableStatement.setLong(2, SrcChargeAmount);
            callableStatement.setLong(3, Tax);
            callableStatement.registerOutParameter(4, OracleTypes.VARCHAR);
            callableStatement.registerOutParameter(5, OracleTypes.INTEGER);

            logger.info("Onelink Topup Bill Payment, executing SP");
            callableStatement.execute();
            String respCode = callableStatement.getString(4);
            Integer spResponse = callableStatement.getInt(5);

            if (spResponse != null && spResponse.equals(1)) {
                logger.error("Approved response received from SP...");
                logger.info("TopupBillPayment in Wallet completed successfully!");
                //wsmodel.setRespcode(respCode);
                return true;
            } else {
                logger.error("Negative response received from SP...");
                if (Util.hasText(respCode)) {
                    logger.error("Response code received from SP [" + respCode + "]");
                    if (respCode.equals(ISOResponseCodes.NP_INVALID_OPERATION)) {
                        logger.error("[Amount minus Charges] is less than zero, rejecting transaction with ref # [" + wsmodel.getTranrefnumber() + "]");
                    }
                } else {
                    logger.error("No response code received from SP, setting [" + ISOResponseCodes.NP_PERMISSION_DENIED + "]");
                    respCode = ISOResponseCodes.NP_PERMISSION_DENIED;
                }
                wsmodel.setRespcode(respCode);
                return false;
            }
