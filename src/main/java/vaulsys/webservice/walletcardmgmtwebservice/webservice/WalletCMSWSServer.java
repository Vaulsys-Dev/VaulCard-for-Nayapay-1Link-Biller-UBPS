...
public class WalletCMSWSServer implements Runnable {
    ...
    public void run() {
        try {
            ...
            //Server server = new Server(GlobalContext.getInstance().getChannelbyId(ChannelCodes.WALLET1LINKBILLERUBPS).getPort());
            //new Server(9005);
            QueuedThreadPool threadPool = new QueuedThreadPool(200, 8, 1000);
            Server server = new Server(threadPool);
            ServerConnector connector = new ServerConnector(server);
            connector.setPort(GlobalContext.getInstance().getChannelbyId(ChannelCodes.WALLET1LINKBILLERUBPS).getPort());
